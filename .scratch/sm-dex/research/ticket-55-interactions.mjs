...
const open = async gen => {
  await page.goto(`${base}?gen=${gen}&open=1&v=55.0`);
...
for(const gen of ids){
  ...
  if(['red','gold','ruby','diamond'].includes(gen))await page.locator('#stage').screenshot({path:`.scratch/sm-dex/research/ticket-55-${gen}-detail.png`});
  ...
  if(['gold','diamond'].includes(gen))await page.locator('#stage').screenshot({path:`.scratch/sm-dex/research/ticket-55-${gen}-closed.png`});
  await page.locator('#toggle').click();
  ...
  for(const width of [768,390,320]){
    ...
    if(width<700){
      ...
      if(width===390&&['gold','diamond'].includes(gen))await page.screenshot({path:`.scratch/sm-dex/research/ticket-55-${gen}-mobile.png`,fullPage:true});
    }
  }
...
await writeFile('.scratch/sm-dex/research/ticket-55-interactions.json',JSON.stringify({originalCapsMatch:true,results,errors},null,2));

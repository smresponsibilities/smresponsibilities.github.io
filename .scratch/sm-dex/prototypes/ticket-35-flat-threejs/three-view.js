...
export function createThree(host,screens,onProjection,options={}){
  ...
  poly(body,scaledDpad,0x350e1d,0,5,2);
  const rocker=new THREE.Group();body.add(rocker);const rockerFace=poly(rocker,dpadOutline,0x36434d,4,11,3);
  const rockerMaterial=rockerFace.material.clone();materials.set(Symbol(),rockerMaterial);rockerFace.material=rockerMaterial;rocker.userData.surface=rockerMaterial;
  const arrowMarks=[[[355,559],[367,559],[361,551]],[[355,613],[367,613],[361,621]],[[335,579],[335,593],[326,586]],[[387,579],[387,593],[396,586]]];
  for(const mark of arrowMarks)poly(rocker,mark,0x111c24,18.2,.6,.15);
  disk(rocker,361,586,6,0x18232b,18.2,.6);caps.set('rocker',rocker);capBase.set('rocker',0);
  ...
  for(const c of controls){if(c.kind==='direction')continue;const parent=c.part==='body'?body:lid,z=c.part==='body'?5:3;let cap;
    if(c.kind.includes('round')){
      ...
      const own=face.material.clone();materials.set(Symbol(),own);face.material=own;cap.userData.surface=own;
    }
  ...
  function display(parent,canvas,area,z){const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.minFilter=THREE.NearestFilter;texture.magFilter=THREE.NearestFilter;texture.generateMipmaps=false;textures.add(texture);screenTextures.push(texture);const material=new THREE.MeshBasicMaterial({map:texture,toneMapped:false});materials.set(Symbol(),material);const geometry=new THREE.PlaneGeometry(area.w,area.h);geometries.add(geometry);const mesh=new THREE.Mesh(geometry,material);mesh.position.set(area.x+area.w/2-HINGE,352-area.y-area.h/2,z);parent.add(mesh);host.dataset.screenSampling='nearest / nearest';}
  ...
  function view(preset){root.rotation.set(preset==='front'?0:-.10,preset==='front'?0:preset==='hinge'?.52:-.22,0);root.scale.setScalar(buttonsOnly||preset==='front'?1:preset==='hinge'?.90:.97);root.position.x=preset==='front'?HINGE-WIDTH/2:0;render();onProjection();}
  ...
  function feedback(id,on){const key=['up','down','left','right'].includes(id)?'rocker':id,cap=caps.get(key);if(cap){cap.userData.surface.emissive.setHex(on?0x4e491d:0x000000);render();}}
  ...
  resize();view(buttonsOnly?'front':'angle');return {resize,pose,view,press,feedback,targets,measure,update,dispose};
}

...
import {createFlat} from './flat.js?v=50';
...
async function setVariant(next){
  ...
  if(next==='three'&&!three){$('hint').textContent='Loading the 3D device…';try{const {createThree}=await import('./three-view.js?v=50');three=createThree($('three-host'),screens,updateTargets);}catch(error){$('hint').textContent='3D unavailable: '+error.message;return;}}
  if(next==='hybrid'&&!hybrid){$('hint').textContent='Loading the hybrid controls…';try{const {createThree}=await import('./three-view.js?v=50');const overlay=createThree($('hybrid-host'),screens,updateTargets,{buttonsOnly:true});hybrid={...overlay,targets:()=>flat.targets(),latchState:()=>flat.latchState(),latchMountState:()=>flat.latchMountState(),latchStraightState:()=>flat.latchStraightState()};}catch(error){$('hint').textContent='Hybrid unavailable: '+error.message;return;}}
  if(next==='shell-flat'&&!shellFlat){$('hint').textContent='Loading the inverse hybrid…';try{const {createThree}=await import('./three-view.js?v=50');shellFlat=createThree($('shell-flat-host'),screens,updateTargets,{flatButtons:true});}catch(error){$('hint').textContent='Inverse hybrid unavailable: '+error.message;return;}}
...
$('run-checks').addEventListener('click',()=>{
  ...
  if(['flat','hybrid'].includes(variant))check('Flat exterior arrow is level',activeRenderer().latchStraightState?.()===true);

...
const MENU = ["PROFILE", "MOVES", "ENCOUNTERS", "RIBBONS", "DEX"];
const VERSIONS = [
  {
    name: "RED",
    entry:
...
function buildControls() {
  ...
  addLayer(bodyControls, asset("stationary-dpad-center-face"), "dpad-center", 324, 618, 40, 42);
  [
    ["up", "D-pad Up", 324, 577, 40, 43, "stationary-dpad-up-face"],
    ["left", "D-pad Left", 282, 618, 43, 42, "stationary-dpad-left-face"],
    ["right", "D-pad Right", 364, 618, 42, 42, "stationary-dpad-right-face"],
    ["down", "D-pad Down", 324, 660, 40, 41, "stationary-dpad-down-face"],
  ].forEach(([action, label, x, y, width, height, face]) =>
    addControl(bodyControls, {
      id: `dpad-${action}`,
      action,
      label,
  ...
  for (let index = 0; index < 10; index += 1) {
    ...
    addControl(innerControls, {
      ...
      label: number <= 5 ? `Open ${MENU[number - 1]}` : `Select list shortcut ${number - 5}`,
      x: 55 + column * 60.8,
      y: 355 + row * 68,
      width: 61,
      height: 68,
...
function keypad(number) {
  if (number <= 5) {
    ...
    const items = currentItems();
    state.itemIndex = Math.min(number - 6, items.length - 1);
    state.mode = "detail";
  }
  render();
}
...
function resizeDevice() {
  const width = Math.min(840, Math.max(1, window.innerWidth - (window.innerWidth <= 480 ? 16 : 24)));
  const scale = width / 840;

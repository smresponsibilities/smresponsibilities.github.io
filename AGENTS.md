...
`BUILD.md` §0 lists the remaining hard rules — no stored images for people, reduced motion
respected everywhere, audio never autoplaying, flavour text short and third-person. §0.0.1 lists
**strong defaults** that used to be hard rules: plain CSS over Tailwind, native Popover over a
tooltip library, CSS and canvas over animation libraries. Those may be overridden with a stated
reason, not by reflex.

**Pokémon assets and React/Next are both permitted** — see `DECISIONS.md` §U. Astro is the
current framework, no longer locked.
